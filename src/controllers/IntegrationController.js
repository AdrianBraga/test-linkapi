...
const { pipedriveApi, blingApi } = require('../services/connectionApi');

module.exports = {
  async index(request, response) {
    const responseApi = await pipedriveApi.get();

    const integration = await responseApi.data.data.map(({ title, status, value, won_time }) => {
      Oportunity.create({
        title,
        status,
        value,
      ...
      blingApi.post(
        `pedido/json/?apikey=${process.env.API_KEY_BLING}&xml=
          ${encodeURI(`
      ...
      .then(() => {
        return response.status(200).json({ status: integration ? `Integracao realizada com sucesso` : 'Integracao nao realizada' })
      })
